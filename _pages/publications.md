...
an identity of Haglund. The title is a description, not a joke. The bijection is completely explicit, though,
...
out the other end. Conditional on the standard conjectures about their Hecke algebras, which is the usual price
of admission. There is [a demo]({{ '/rational-catalan/' | relative_url }}) that builds the same number from
both ends at once.
...
refinement, where that count is upgraded to a *q*-series coming from orbit harmonics. We prove it for
hypersimplices &mdash; the slices of a cube &mdash; by writing down generators for the harmonics ideal, which as
